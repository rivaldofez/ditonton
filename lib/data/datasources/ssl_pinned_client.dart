...
class HttpSSL {
  static Future<http.Client> get _instance async =>
      _clientInstance ??= await HttpUtils.createLEClient();
...
class HttpUtils {
  static Future<HttpClient> customeHttpClient({bool isTestMode = false}) async {
    ...
    try {
      ...
      if (isTestMode) {
        ...
        try {
          certFileBytes = (await rootBundle.load('assets/certificates.pem'))
              .buffer
              .asInt8List();
  ...
  static Future<http.Client> createLEClient({bool isTestMode = false}) async {
    IOClient client =
        IOClient(await HttpUtils.customeHttpClient(isTestMode: isTestMode));
    return client;
  }
}
